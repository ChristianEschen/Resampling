...
print "is raw input file valid:"
print os.path.exists(args.PETraw_path)
print "is dicom csi input file valid:"
print os.path.exists(args.dicom_path)

output=args.PETdcm

new_img = np.fromfile(args.PETraw_path, dtype='float64', sep="")
new_img = new_img.reshape([int(np.sqrt(len(new_img))), int(np.sqrt(len(new_img)))])

ds = dicom.read_file(args.dicom_path)
